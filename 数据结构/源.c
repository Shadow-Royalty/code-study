...
//		L->next = p;
// 
// 头插法就是让第一个p做第一个结点，保证p的完整然后将p插在L的前面，所以才会倒序
//		
//	return L;
//}
//
...
//	return L;
//}
//r=L与r=p的作用是调位，在循环中让下一个数据再永远的指向尾的位置，这样就能保证第一个数据永远在第一个
// 少了这个指针就是头插法，这样就是倒序
...
linklist* creat_list(int* count)
{
	...
	/*	while (scanf("%d", &x) != EOF)*/
		...
		return head;//头结点和头插法一定要返回给链表！

	//头/尾插法一般都是用于一次性插入大量数据，一次的一般用不上
		//但是完全就可以代替倒序插入，直接这么做就ok，这就是头插法

}

void show(linklist* head)
{
	linklist* p = head->next;//这里因为head一开始一定是空的（头结点），所以要先指向下一个位置防止乱报
	if (p == NULL)//不用指向数据，是有没有结点，而不是数据有没有！不然在while时就死循环了
	{
	...
	else
	{
		...
		{
			printf("%d ", p->data);
			p = p->next;//下一个都忘了，你是懂死循环的。
		}
		printf("\n");
	}
	
}
void Insert(linklist* head,int a,int x,int* count)//位置和新数据
{
	linklist* pre, *p;
	pre = head;//直接相等就等于指向
	int t;
	/*找到第t个位置之前的结点即第a-1个结点*/
	 /* for（int i=0;i<t-1;i++) */
	for (t = 1; t < a; t++)
	{
		pre = pre->next;
	...
	p->next = pre->next;//当头插法去理解就行了，先两个连接起来，然后在赋值(但凡是插入）
	pre->next = p;
	/*结点数加1*/
	(*count)++;
	printf("insert OK\n");
...
{
	linklist* pre, * p = head;//pre使用时没分配空间,结点使用都要分配！
	//之前一直总是跳过一个就在于你手贱*p=head->next,不是头指针你一开始就不用手贱指向下一个位置！
	pre = malloc(sizeof(linklist));

	int i;
	
for(i=0;i<n;i++)
		//你这个是查找某个元素，而题目要你做的是在哪个位置的值！
	{
		pre = p;
		p = p->next;
	}
